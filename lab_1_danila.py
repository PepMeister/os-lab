...
class Dispatcher(): #Task):
	def __init__(self, tasks):
		...
		self.quantum = 4
		self.endOfquantumTime = self.currentTask.execTime
	...
	def skip(self, num):
		for i in range(num):
			print(str(self.time) + " " + "-")
			self.time += 1
			self.endOfquantumTime = self.currentTask.actTime + self.quantum
	...
	def check_current_task(self):
		if self.currentTask.endOfExecutionTime == self.time:
			...
			self.endOfquantumTime = self.time + self.quantum


		if self.endOfquantumTime == self.time:
			print("истек квант процессорного времени, выполняется следующая задача" )
			...
			for task in self.task_list:
				try:
					...
					num = self.currentTask.actTime - self.time
					#print("was", self.endOfquantumTime)
					#if num != 0: 
					#self.endOfquantumTime = self.currentTask.actTime + self.quantum
					#print("now", self.endOfquantumTime)
					self.skip(num)
			self.currentTask.endOfExecutionTime = self.time + self.currentTask.execTime
			self.endOfquantumTime = self.time + self.quantum
...
def run_modeling(tasks):
	...
	print("квант процессорного времени равен: ", dispatcher.quantum, "\n" )
